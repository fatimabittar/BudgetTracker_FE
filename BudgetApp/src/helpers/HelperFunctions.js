...
import { categoryColors } from '../utils/globalStyle.js';
...
const eliminateReminders = (categories) => {
  ...
  for (const category of categories) {
    ...
    for (const trans of category.transactions) {
...
const netExpense = (categories) => {
  let total = 0;
  for (const category of categories) total += category.totalExpense;
  return total;
};
...
const monthlyExpensesOfLastYear = (transactions) => {
  ...
  for (const item of transactions) {
    ...
    const month = moment(item.transactionDate).format('MMM');
...
const lastNMonthsExpenses = (allExpenses, numberOfMonths) => {
  ...
  for (let i = 0; i < numberOfMonths; i++) {
    const month = monthNames[currentMonthNo];
    result.months.push(month);
    if (allExpenses.hasOwnProperty(month)) result.expenses.push(allExpenses[month]);
